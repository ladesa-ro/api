    ...
    (function() {
      ...
      var runInContext = function runInContext2(context) {
        ...
        function times(n4, iteratee2) {
          ...
          var index = MAX_ARRAY_LENGTH, length = nativeMin(n4, MAX_ARRAY_LENGTH);
...
// plop/helpers/change-case.helper.ts
var ChangeCaseHelper = {
  c_camel: camelCase,
  c_capital: capitalCase,
  c_constant: constantCase,
  c_dot: dotCase,
  c_kebab: kebabCase,
  c_no: noCase,
  c_pascal: pascalCase,
  c_pascalSnake: pascalSnakeCase,
  c_path: pathCase,
  c_sentence: sentenceCase,
  c_snake: snakeCase,
  c_train: trainCase
};
var ChangeCaseHelperPlop = (plop) => {
  plop.setHelper("c_camel", camelCase);
  plop.setHelper("c_capital", capitalCase);
  plop.setHelper("c_constant", constantCase);
  plop.setHelper("c_dot", dotCase);
  plop.setHelper("c_kebab", kebabCase);
  plop.setHelper("c_no", noCase);
  plop.setHelper("c_pascal", pascalCase);
  plop.setHelper("c_pascalSnake", pascalSnakeCase);
  plop.setHelper("c_path", pathCase);
  plop.setHelper("c_sentence", sentenceCase);
  plop.setHelper("c_snake", snakeCase);
  plop.setHelper("c_train", trainCase);
  return plop;
};

// plop/helpers/index.ts
function Helpers(plop) {
  ChangeCaseHelperPlop(plop);
  plop.setHelper("eq", (a, b5) => a === b5);
  ...
  plop.setHelper("ternary", (cond, a, b5) => cond ? a : b5);
  plop.setHelper("append", (a) => a);
}

// plop/generators/module-core/generators/BaseModuleCoreGenerator.ts
import { generateCode, parseModule } from "magicast";

// plop/helpers/ts-ast/ts-ast.ts
...
// plop/helpers/ts-ast/ts-ast.ts
var findNestJsModuleObjectConfigProperty = (ast, propName) => new Promise((resolve2) => {
  visit(ast, {
    visitClassDeclaration(path2) {
      visit(path2.node.decorators, {
        visitObjectProperty(path3) {
          const node = path3.node;
          if (n.Identifier.assert(node.key)) {
            if (node.key.name === propName) {
              const parentsToCheck = [
                //
                [null, (path4) => path4.parentPath],
                [() => true, (path4) => path4.parentPath],
                [(path4) => n.ObjectExpression.assert(path4.node), (path4) => path4.parentPath],
                [(path4) => n.CallExpression.assert(path4.node), (path4) => path4.parentPath.parentPath],
                [(path4) => n.Decorator.assert(path4.node), (path4) => path4],
                [
                  (path4) => {
                    const decoratorNode = path4.node;
...
var addImportMember = (mod, path2, member) => {
  if (mod.imports.$items.some((i) => i.from === path2 && i.imported === member)) {
    return;
  }
  mod.imports.$add({ from: path2, imported: member });
};
var checkHasExportAllFromPathName = (ast, pathName) => {
  return new Promise((resolve2) => {
    visit(ast, {
      visitExportAllDeclaration(path2) {
...
var addExportAllFrom = async (mod, pathName) => {
...
var getClassNode = ($ast, className) => {
  return new Promise((resolve2) => {
    visit($ast, {
      visitClassDeclaration(path2) {
        const node = path2.node;
...
var addPropertyAcessor = async (ast, className, accessorName, property) => {
  const classNode = await getClassNode(ast, className);
  if (!classNode) {
    throw new Error(`N\xE3o foi poss\xEDvel encontrar ${className}.`);
  }
  const acessorAlreadyExists = await new Promise((resolve2) => {
    visit(classNode, {
      visitClassMethod(path2) {
        const node = path2.node;
...
// plop/generators/module-core/generators/BaseModuleCoreGenerator.ts
var BaseModuleCoreGenerator = class {
  constructor() {
    this.modifyModuleQueue = [];
  }
  get isDirty() {
  ...
  addModifyAcessor(className, accessorName, property) {
    this.addModify(async (mod) => {
      await addPropertyAcessor(mod.$ast, className, accessorName, property);
    });
    return this;
  }
  addModifyCode(callback) {
    this.modifyModuleQueue.push({ kind: "code", fn: callback });
    return this;
  }
  addImportMember(path2, member) {
    if (path2) {
      this.addModify((mod) => addImportMember(mod, path2, member));
    }
    return this;
  }
  addExportAllFrom(path2) {
    this.addModify((mod) => addExportAllFrom(mod, path2));
    return this;
  }
  async transform(fileModuleContent) {
...
// plop/generators/module-core/generators/ModuleCoreGeneratorAuthzStatement.ts
import { builders as b2 } from "ast-types";
import { parseModule as parseModule2 } from "magicast";
var ModuleCoreGeneratorAuthzStatement = class extends BaseModuleCoreGenerator {
  addTypeDeclarationStatement(typeName, actionName, type, dto = null) {
    this.addModify(async (mod) => {
      ...
      if (mod.$ast.type === "Program") {
        ...
        const statementDeclarationMod = parseModule2(statementDeclaration);
        mod.$ast.body.splice(indexOfStatementCheck, 0, statementDeclarationMod.$ast.body[0]);
        ...
        if (nodeToAdd) {
          const typeDeclaration = nodeToAdd.declaration;
          const annotation = typeDeclaration.typeAnnotation;
          if (annotation.type === "TSUnionType") {
            annotation.types.push(b2.tsTypeReference(b2.identifier(typeName)));
          }
        }
      }
    });
...
// plop/generators/module-core/generators/ModuleCoreGeneratorDatabaseContextCore.ts
import { parseModule as parseModule3 } from "magicast";
var ModuleCoreGeneratorDatabaseContextCore = class extends BaseModuleCoreGenerator {
  addRepository(moduleName, accessorName = `${ChangeCaseHelper.c_camel(moduleName)}Repository`, createRepositoryName = `create${ChangeCaseHelper.c_pascal(moduleName)}Repository`) {
    const dummyCode = `export class Dummy {
      get ${accessorName}() {
        return repositories.${createRepositoryName}(this.ds);
      }
    }`;
    const dummyMod = parseModule3(dummyCode);
    const property = dummyMod.$ast.body[0].declaration.body.body[0];
    this.addModifyAcessor("DatabaseContextCore", accessorName, property);
    return this;
  }
};

// plop/generators/module-core/generators/ModuleCoreGeneratorNestModule.ts
var import_lodash = __toESM(require_lodash());
import { builders as b3, namedTypes as n3 } from "ast-types";
var ModuleCoreGeneratorNestModule = class extends BaseModuleCoreGenerator {
  addItemToModule(moduleDeclarationProperty, importMember, importPath = null) {
    this.addModify(async (mod) => {
      if (importPath) {
      ...
      if (property) {
        if (n3.ArrayExpression.assert(property.value)) {
          ...
          if (!alreadyDeclared) {
            property.value.elements.push(b3.identifier(importMember));
          }
        }
      }
    });
  }
  appendModuleConfig(moduleDeclareClass) {
    for (const property of (0, import_lodash.castArray)(moduleDeclareClass.declareInModuleProperty)) {
...
// plop/generators/module-core/generators/ModuleCoreGeneratorOperations.ts
import { builders as b4, visit as visit2 } from "ast-types";
var ModuleCoreGeneratorOperations = class extends BaseModuleCoreGenerator {
  addOperation(token, importPath) {
      ...
      visit2(mod.$ast, {
        visitObjectExpression(path2) {
          ...
          if (!isPropertyAlreadyDeclared) {
            node.properties.push(b4.objectProperty(b4.identifier(token), b4.identifier(token)));
          }
          return false;
        }
      });
...
// plop/generators/module-core/questions/moduleCorePromptQuestions.ts
import inq from "inquirer";
import { basename as basename2, dirname as dirname2 } from "path";
var askConfirm = async (inquirer, msg, defaultValue = true) => {
  const aws = await inquirer.prompt([{ name: "confirm", type: "confirm", message: msg, default: defaultValue }]);
  return aws.confirm;
};
var ENABLE_MOCK_PROFESSOR = false;
async function moduleCorePromptQuestions(inquirer) {
  ...
  const moduleNameParent = basename2(dirname2(modulePath));
  const moduleNameParentKebab = kebabCase(moduleNameParent);
  ...
  const { estrutura } = await inquirer.prompt([
    {
      type: "checkbox",
      name: "estrutura",
      message: "Estrutura do m\xF3dulo",
      choices: [
        {
  ...
  const { operacoes } = await inquirer.prompt([
    {
      type: "checkbox",
      name: "operacoes",
      message: "A\xE7\xF5es para incluir",
      when: () => estrutura.includes("model"),
      default: estrutura.includes("model") ? void 0 : [],
      choices: [
        {
  ...
  const { database } = await inquirer.prompt([
    {
      type: "checkbox",
      name: "database",
      message: "Integra\xE7\xE3o ao banco de dados.",
      when: () => estrutura.includes("model"),
      default: estrutura.includes("model") ? void 0 : [],
      choices: [
    ...
    do {
      ...
      const { nome, descricao, tipoInterface, nullable, nomeColuna, tipoSwagger, tipoGraphQl, tipoEntidadeColuna, tipoEntidadeInterface } = await inquirer.prompt([
        ...
        },
        {
          name: "descricao",
          type: "input",
          message: `----- Propriedade #${count} - Descri\xE7\xE3o:`
        },
        {
          name: "tipoInterface",
          type: "input",
        ...
        },
        {
          name: "tipoSwagger",
          type: "input",
          message: `----- Propriedade #${count} - [SWAGGER] - Tipo da propriedade na declara\xE7\xE3o do dto swagger (LEMBRE DAS ASPAS):`
        },
        {
          name: "tipoGraphQl",
          type: "input",
          message: `----- Propriedade #${count} - [GRAPHQL] - Tipo da propriedade na declara\xE7\xE3o do dto graphql:`
        },
        {
          name: "nullable",
          type: "confirm",
          message: `----- Propriedade #${count} - Coluna anul\xE1vel?`,
          default: false
        },
        {
          name: "nomeColuna",
          ...
          default: (aws2) => ChangeCaseHelper.c_snake(aws2.nome)
        },
        {
          ...
          default: (aws2) => aws2.tipoInterface
        }
      ]);
      if (await askConfirm(inq, "Confirma a declara\xE7\xE3o desta propriedade?", false)) {
...
// plop/generators/module-core/module-core.generator.ts
var timestamp = "0000000000000";
var ModuleCoreGenerator = {
  description: "M\xF3dulo Gen\xE9rico.",
  ...
  actions: (data) => {
    ...
    actions.push({
      type: "modify",
      path: `src/application/business/{{ c_kebab moduleNameParent }}/{{ c_kebab moduleNameParent }}.module.ts`,
      transform: async (code) => new ModuleCoreGeneratorNestModule().appendModuleConfig({
    ...
    const moduleEstruturaTransforms = [
      {
    ...
    if (answers.operacoes.includes("handle-resource-create") || answers.operacoes.includes("handle-resource-update")) {
      ...
      if (answers.operacoes.includes("handle-resource-create")) {
        ...
        actions.push({
          type: "modify",
          path: `${outputPathModule}/dtos/${ChangeCaseHelper.c_kebab(answers.moduleName)}.operations.ts`,
          transform: async (code) => new ModuleCoreGeneratorOperations().addOperation(`${ChangeCaseHelper.c_constant(answers.moduleName)}_CREATE`, `./${ChangeCaseHelper.c_kebab(answers.moduleName)}-create.operation`).transform(code)
        });
        actions.push({
          type: "modify",
          path: `src/application/authorization-policies/statements/IAuthzStatement.ts`,
          transform: async (code) => new ModuleCoreGeneratorAuthzStatement().addTypeDeclarationStatement(
      ...
      if (answers.operacoes.includes("handle-resource-update")) {
        ...
        actions.push({
          type: "modify",
          path: `${outputPathModule}/dtos/${ChangeCaseHelper.c_kebab(answers.moduleName)}.operations.ts`,
          transform: async (code) => new ModuleCoreGeneratorOperations().addOperation(`${ChangeCaseHelper.c_constant(answers.moduleName)}_UPDATE`, `./${ChangeCaseHelper.c_kebab(answers.moduleName)}-update.operation`).transform(code)
        });
        actions.push({
          type: "modify",
          path: `src/application/authorization-policies/statements/IAuthzStatement.ts`,
    ...
    if (answers.operacoes.includes("handle-resource-delete")) {
      ...
      actions.push({
        type: "modify",
        path: `${outputPathModule}/dtos/${ChangeCaseHelper.c_kebab(answers.moduleName)}.operations.ts`,
        transform: async (code) => new ModuleCoreGeneratorOperations().addOperation(`${ChangeCaseHelper.c_constant(answers.moduleName)}_DELETE_ONE_BY_ID`, `./${ChangeCaseHelper.c_kebab(answers.moduleName)}-delete-one.operation`).transform(code)
      });
      actions.push({
        type: "modify",
        path: `src/application/authorization-policies/statements/IAuthzStatement.ts`,
    ...
    if (answers.operacoes.includes("handle-resource-read")) {
      ...
      actions.push({
        type: "modify",
        path: `${outputPathSpec}/operations/index.ts`,
        transform: async (code) => new BaseModuleCoreGenerator().addExportAllFrom(`./${ChangeCaseHelper.c_kebab(answers.moduleName)}-find-all`).addExportAllFrom(`./${ChangeCaseHelper.c_kebab(answers.moduleName)}-find-one`).transform(code)
      });
      actions.push({
        type: "add",
      ...
      actions.push({
        type: "modify",
        path: `${outputPathModule}/dtos/index.ts`,
        transform: async (code) => new BaseModuleCoreGenerator().addExportAllFrom(`./${ChangeCaseHelper.c_kebab(answers.moduleName)}-find-all.operation`).addExportAllFrom(`./${ChangeCaseHelper.c_kebab(answers.moduleName)}-find-one.operation`).transform(code)
      });
      actions.push({
        type: "modify",
        path: `${outputPathModule}/dtos/${ChangeCaseHelper.c_kebab(answers.moduleName)}.operations.ts`,
        transform: async (code) => new ModuleCoreGeneratorOperations().addOperation(`${ChangeCaseHelper.c_constant(answers.moduleName)}_FIND_ALL`, `./${ChangeCaseHelper.c_kebab(answers.moduleName)}-find-all.operation`).addOperation(`${ChangeCaseHelper.c_constant(answers.moduleName)}_FIND_ONE_BY_ID`, `./${ChangeCaseHelper.c_kebab(answers.moduleName)}-find-one.operation`).transform(code)
      });
      actions.push({
        type: "modify",
        path: `src/application/authorization-policies/statements/IAuthzStatement.ts`,
        transform: async (code) => new ModuleCoreGeneratorAuthzStatement().addTypeDeclarationStatement(`IAuthzStatement${ChangeCaseHelper.c_pascal(answers.moduleName)}Filter`, `${ChangeCaseHelper.c_snake(answers.moduleName)}:find`, "filter", null).transform(code)
      });
    }
    return actions;
  }
