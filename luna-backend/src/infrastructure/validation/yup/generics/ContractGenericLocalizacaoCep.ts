import { ContractGenericString } from '../../..';

const CEP_PATTERN = /^((\d{5}-\d{3})|(\d{8}))$/;

type IContractGenericLocalizacaoCepOptions = {
  required?: boolean | string;
  output?: 'clean' | 'formatted' | 'passthrough';
};

export const ContractGenericLocalizacaoCep = (
  options: IContractGenericLocalizacaoCepOptions = {},
) => {
  const { required = false, output = 'formatted' } = options;

  let base = ContractGenericString.transform((value) => {
